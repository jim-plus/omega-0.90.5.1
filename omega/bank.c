...
static void bank_deposit (WINDOW *w, bank_account *account, pob card)
{
  ...
  if (amount < 0)
  ...
  else
    {
      ...
      if (card->blessing > 0)
        {
          mvwprintw(w, 14, 2, "I'll take good care of %ld AU for you!", amount);
        }
      else if (card->blessing < 0)
        {
          fee = 1 + random_range(amount) / 2;
          mvwprintw(w, 14, 2, "Minus my fee, that makes %ld AU for you...", amount - fee);
        }
      else
        {
          mvwprintw(w, 14, 2, "%ld AU deposited", amount);
        }
  ...
  if (amount < 0)
    {
      ...
      mvwaddstr(w, 14, 2, blank_line);
      if (card->blessing > 0)
        mvwprintw(w, 14, 2, "Whoa, thanks for the %ld AU!", amount);
      else if (card->blessing < 0)
        mvwprintw(w, 14, 2, "Sucker! %ld AU is mine! all mine!", amount);
      else
        mvwprintw(w, 14, 2, "%ld AU deposited", amount);
      ...
      if (card->blessing > 0)
        {
          if (account->balance > 0)
            mvwprintw(w,  6, 2, "Say pal, you've got: %ld AU!", account->balance);
          else
            mvwprintw(w,  6, 2, "Oh my, you've got: %ld AU.", account->balance);
        ...
        {
          if (account->balance > 0)
            mvwprintw(w,  6, 2, "Muahaha! I've got %ld AU!", account->balance);
          else if (account->balance < 0)
            mvwprintw(w,  6, 2, "Muahaha! You're owe me %ld AU!", -account->balance);
          else
        ...
        {
          mvwprintw(w,  6, 2, "Current balance: %ld AU", account->balance);
